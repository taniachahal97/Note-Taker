...
// Show an element
const show = (elem) => {
  elem.style.display = 'inline'; //been used 
};

// Hide an element
const hide = (elem) => {
  elem.style.display = 'none'; // been used 
};
...
const handleNoteSave = () => {
  ...
  saveNote(newNote).then(() => {
    ...
    //activeNote.id = data.body.id;
    //handleNoteView(newNote);
    getAndRenderNotes();
    renderActiveNote();
  });
};
...
const handleRenderSaveBtn = () => {
  if (noteTitle.value || noteText.value) {
    show(saveNoteBtn);
  } else if (!noteTitle.value && !noteText.value) {
    hide(saveNoteBtn);
  }
